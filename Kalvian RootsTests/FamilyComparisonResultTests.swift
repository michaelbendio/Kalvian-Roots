...
final class FamilySearchDOMServiceTests: XCTestCase {
    ...
    func testAtlasExtractorIncludesCallbackAndCardLineParser() {
        ...
        XCTAssertTrue(script.contains("function extractChildDetailsFromPersonPage(summary)"))
        XCTAssertTrue(script.contains("const person = await visitPerson(summary.id);"))
        XCTAssertTrue(script.contains("function childDetailHasVitalDates(detail)"))
