...
final class FamilyComparisonResultTests: XCTestCase {
    ...
    func testFamilySearchMatsMatchesJuuretMattiOnSameBirthDate() throws {
        nameManager.addEquivalence(between: "Matti", and: "Mats")

        let result = FamilyComparisonResult(
            familySearch: [
                candidate(
                    name: "Mats",
                    birth: date(1806, 9, 17),
                    source: .familySearch
                )
            ],
            juuretKalvialla: [
        ...
        let match = try XCTUnwrap(result.matches.first)
        XCTAssertEqual(match.familySearch?.rawName, "Mats")
...
final class FamilySearchDOMServiceTests: XCTestCase {
    ...
    func testServerRenderedHiskiOnlyChildDateLinksParentlessBirthQuery() {
        let nameManager = NameEquivalenceManager()
        nameManager.clearAllEquivalences()
        let family = Family(
        ...
        XCTAssertTrue(html.contains("etunimi=Sophia"))
        XCTAssertTrue(html.contains("alkuvuosi=21.11.1821"))
    ...
    func testServerRenderedCitationCopyButtonWritesCitationToClipboard() {
        let family = Family(
            familyId: "SAKERI 1",
            pageReferences: ["1"],
            husband: Person(name: "Matti"),
        ...
        XCTAssertTrue(html.contains("onclick=\"copyCitation()\""))
    ...
    func testServerRenderedBirthDateLinksOpenParentlessHiskiResultsTab() {
        let family = Family(
            familyId: "SAKERI 1",
            pageReferences: ["264", "265"],
    ...
    func testHiskiBirthWorkbenchRendersEditableQueryAndMagnifierCitationRows() throws {
        ...
        XCTAssertTrue(html.contains(#"<form id="hiskiBirthForm""#))
        XCTAssertTrue(html.contains(#"method="POST""#))
        XCTAssertTrue(html.contains(#"action="/family/RITA%20II%204/hiski-birth-search""#))
        XCTAssertTrue(html.contains(#"<a class="family-workspace-action" href="/family/RITA%20II%204">Cancel</a>"#))
        XCTAssertTrue(html.contains(#"name="refreshFamily""#))
        XCTAssertTrue(html.contains(#"data-family-refresh="1">Family</button>"#))
        ...
        XCTAssertTrue(html.contains("if (clickedButton && clickedButton.dataset.familyRefresh)"))
        let familyRefreshIndex = try XCTUnwrap(html.range(of: "if (clickedButton && clickedButton.dataset.familyRefresh)")?.lowerBound)
        let preventDefaultIndex = try XCTUnwrap(html.range(of: "event.preventDefault();")?.lowerBound)
        XCTAssertLessThan(familyRefreshIndex, preventDefaultIndex)
    ...
    func testFamilySearchExtractorWaitsForFamilyMembersSectionsBeforeReadingChildren() {
        ...
        XCTAssertTrue(script.contains("function familyMembersLines()"))
        XCTAssertTrue(script.contains("function linesContainFamilyRows(lines)"))
        XCTAssertTrue(script.contains("function spousesAndChildrenIndex(lines)"))
        XCTAssertTrue(script.contains("function personIdFromHref(href)"))
        ...
        XCTAssertTrue(script.contains("function linkFallbackDiagnostics(expectedPersonId)"))
        XCTAssertTrue(script.contains("function familyMembersLineDiagnostics()"))
        XCTAssertTrue(script.contains("FamilySearch spouse section diagnostics: lines="))
        XCTAssertTrue(script.contains("FamilySearch person-link diagnostics: root="))
        XCTAssertTrue(script.contains("FamilySearch visible line diagnostics: sample="))
        XCTAssertTrue(script.contains("FamilySearch element diagnostics: sample="))
        XCTAssertTrue(script.contains("const elementSamples = elements.slice(0, 18).map(element => {"))
